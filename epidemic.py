...
class EpidemicEnv(object):
	...
	#NOTE: Changed budget_c to a scalar rather than a percentage
	def __init__(self, graph, budget_c=5, Initial_I=0.1, infect_prob=0.05, cure_prob=0.05):
		self.graph=graph
		self.n=len(graph)
		...
		#dictionary of infected individuals at a given time
		self.inf_t = {}
	...
	def reset(self):
		self.all_nodes= list(range(self.n))
		self.true_state = np.zeros(self.n)
		#Will reset the states and set a fixed percentage of individuals to be initially infected
