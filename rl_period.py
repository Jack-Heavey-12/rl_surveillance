...
class DQN(FQI):
	...
	def fit_GCN(self, num_iterations=100, num_epochs=100, eps=0.1, discount=0.9):
		...
		for epoch in range(num_epochs):
			...
			for episode in range(num_iterations):
				print(f'Running episode {episode} of run_episode_GCN()')
	...
	def run_episode_GCN(self, eps=0.1, discount=1):
		print(f'Beginning of run_episode_GCN()')
		S, A, R = [], [], []
		cumulative_reward = 0
		a=0
		self.simulator.reset()
		for t in range(self.simulator.T-1):
			print(f'Episode {t} of run_episode_GCN()')
			state = self.simulator.belief_state.copy()
			S.append(state)
			action = self.policy_GCN(state, eps)
			true_state, state_, reward, true_reward = self.simulator.perform(action, t, final_iteration=False, period=5)
			A.append(action)
			R.append(reward)
			cumulative_reward += reward * (discount**t)
		state = self.simulator.belief_state.copy()
		S.append(state)
		action = self.policy_GCN(state, eps)
		true_state, state_, reward, true_reward = self.simulator.perform(action, self.simulator.T, final_iteration=True, period=1)
		A.append(action)
		R.append(reward)
		cumulative_reward += reward * (discount**t)
