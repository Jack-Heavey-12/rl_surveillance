...
class EpidemicEnv(object):
	...
	def perform(self, iso_list, t, final_iteration = False, period = 1):
...
if __name__ == '__main__':
	...
	for i in range(env.T):
		...
		reward = env.perform(action, i, final_iteration=False, period=5)
		#true_I.append(sum(S1))
		Reward.append(reward[2])
	action = random.sample(env.all_nodes,min(len(env.all_nodes),env.budget))
	reward = env.perform(action, i, final_iteration=True, period=1)
	Reward.append(reward[2])
